...
// Чтобы исправить эту ошибку, мы можем использовать bind для привязки контекста payment к методу getDate, как показано выше. Теперь при вызове User.paymentDate() this будет указывать на payment, и метод getDate сможет корректно вернуть дату платежа.